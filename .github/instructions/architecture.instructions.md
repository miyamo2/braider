---
applyTo: "internal/analyzer/**,internal/registry/**,cmd/braider/main.go"
---
...
- `internal/analyzer/integration_test.go` / `internal/analyzer/testdata/**` (golden + checkertest workflow) → see `testing.instructions.md`
- Constructor / bootstrap AST construction itself (the pipeline only invokes it; emit details live elsewhere) → see `code-generation.instructions.md`
- Diagnostic wording, `SuggestedFix` structure, severity mapping → see `code-generation.instructions.md`
- Annotation / option type specifications themselves (the analyzer consumes them but does not define them) → see `annotations.instructions.md`
