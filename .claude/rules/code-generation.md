...
- When and in which phase generation is invoked (pipeline coordination) → see `architecture.md`
- Golden / checkertest validation workflow for emitted code → see `testing.md`
- Detection logic that feeds inputs into generation (annotation detection, tag parsing) → see `annotations.md` / `struct-tags.md`
