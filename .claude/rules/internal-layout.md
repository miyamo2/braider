...
This file's `paths:` is `internal/**`, the broadest in the project. It is intentionally a one-line-per-package index only; every detailed topic below is delegated to a topic-specific rule:

- Analyzer coordination details → see `architecture.md`
- Detector / annotation mapping → see `annotations.md`
- Tag handling details → see `struct-tags.md`
- `internal/generate/` / `internal/report/` internals → see `code-generation.md`
- `integration_test.go` / `testdata/` layout → see `testing.md`
