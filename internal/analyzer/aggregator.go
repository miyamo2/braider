...
func NewAggregator(providerRegistry *registry.ProviderRegistry, injectorRegistry *registry.InjectorRegistry, variableRegistry *registry.VariableRegistry, duplicateRegistry *registry.DuplicateRegistry, entryPointRegistry *registry.EntryPointRegistry) *Aggregator {
	return &Aggregator{
...
func (a *Aggregator) AfterDependencyPhase(graph *checker.Graph) error {
	for act := range graph.All() {
		...
		// Entry-point aggregation: record main-package and explicit-App presence so
		// AppAnalyzeRunner can infer annotation.App for single-main-package projects
		// or emit the multi-main ambiguity diagnostic when no explicit App exists.
		if a.EntryPointRegistry != nil {
