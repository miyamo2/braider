...
func compareGolden(t *testing.T, graphs []*gochecker.Graph, gf *goldenFiles) {
	t.Helper()
			...
			for _, d := range act.Diagnostics {
				for i := range d.SuggestedFixes {
					...
					for _, edit := range fix.TextEdits {
						...
						allFixEdits = append(allFixEdits, fixEdit{
							message: fix.Message,
							file:    file.Name(),
...
func compareAllFixesGolden(t *testing.T, filename string, original []byte, ffe *fileEditsInfo, expected []byte) {
	t.Helper()
...
func compareArchiveGolden(t *testing.T, filename string, original []byte, ffe *fileEditsInfo, ar *txtar.Archive) {
	t.Helper()
