...
func collectExpectations(t *testing.T, pkgs []*packages.Package) map[expectKey][]*expectation {
	t.Helper()
...
func checkDiagnostics(t *testing.T, wants map[expectKey][]*expectation, posn token.Position, message string) {
	t.Helper()
	...
	if len(expects) > 0 {
		...
		t.Errorf("%s:%d: diagnostic %q does not match any of %s",
...
func reportUnmatched(t *testing.T, wants map[expectKey][]*expectation) {
	t.Helper()
	for k, expects := range wants {
		for _, exp := range expects {
			t.Errorf("%s:%d: no diagnostic was reported matching %q",
