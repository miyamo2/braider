...
func DiffBytes(a, b []byte) []Diff { return computeDiff(bytesSeqs{a, b}) }

// DiffRunes returns the differences between two rune sequences.
func DiffRunes(a, b []rune) []Diff { return computeDiff(runesSeqs{a, b}) }

// DiffLines returns the differences between two string sequences.
func DiffLines(a, b []string) []Diff { return computeDiff(linesSeqs{a, b}) }

// A limit on how deeply the LCS algorithm should search. The value is just a guess.
var maxDiffs = 100

func computeDiff(seqs sequences) []Diff {
