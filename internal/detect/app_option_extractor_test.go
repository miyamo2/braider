...
func TestAppOptionExtractor_NilMarkers(t *testing.T) {
	extractor := detect.NewAppOptionExtractorImpl(nil)
	...
	tests := []struct {
		...
		{
			name: "app.Default with nil markers falls back to IsDefault",
			src: `package main
...
`,
		...
		{
			name: "app.Container with nil markers falls back to unsupported",
			src: `package main
...
`,
	...
	pkgs, _ := createAppOptionTestPackages()
	markers, err := detect.ResolveMarkers()
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			pass, _ := mockPassForAppOption(t, tt.src, pkgs)

			detector := detect.NewAppDetector(markers)
			apps := detector.DetectAppAnnotations(pass)

			if len(apps) == 0 {
				// If no apps detected (type checking may fail), just verify no panic
				return
			}

			// Should not panic even with nil markers
			metadata, err := extractor.ExtractAppOption(pass, apps[0])
			// With nil markers, neither isAppDefaultOption nor isAppContainerOption will match
			// so unknown option type may produce an error
			_ = metadata
			_ = err
		})
	}
...
func TestAppOptionExtractor_NilSpecificMarkerField(t *testing.T) {
	// MarkerInterfaces with all fields nil
	emptyMarkers := &detect.MarkerInterfaces{}
	extractor := detect.NewAppOptionExtractorImpl(emptyMarkers)
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Should not panic with nil specific marker fields
			metadata, err := extractor.ExtractAppOption(pass, apps[0])
			_ = metadata
			_ = err
		})
